...
//	public static void bpy_context_set(bContext C, PyGILState_STATE gilstate)
	public static void bpy_context_set(bContext C)
	{
//		py_call_level++;
//
//		if(gilstate)
...
//		if(py_call_level==1) {
			...
			if(C!=null) { // XXX - should always be true.
				BpyUtil.BPy_SetContext(C);
//				bpy_import_main_set(CTX_data_main(C));
			}
			else {
				System.err.printf("ERROR: Python context called with a NULL Context. this should not happen!\n");
			}
...
//	// TMP
//	private static final String uiPath = "/release/scripts/ui/";
...
//		"properties_material",
//	};
//	private static final String modulePath = "/release/scripts/modules/";
//	private static String[] moduleDir = {
//		"rna_prop_ui",
//	};

	/* call BPY_set_context first */
	public static void BPY_start_python( int argc, String[] argv )
	{
...
//			PyDict_SetItemString(d, "__import__",	item=PyCFunction_New(&bpy_import_meth, NULL));	Py_DECREF(item);
			...
			// TMP
			bpy_import_meth(Bpy.modulePath, Bpy.moduleDir);
			bpy_import_meth(Bpy.uiPath, Bpy.uiDir);
...
//	// TMP
//	private static void bpy_import_meth(String pathname, String[] modnames) {
//		PyList list = new PyList();
////		list.add(Util.BLI_gethome() + pathname);
//		for (String modname : modnames) {
//			URL url = ResourceAnchor.class.getClassLoader().getResource("resources"+pathname);
//			if (url != null) {
//				System.out.println("resource found: "+"resources"+pathname);
//			}
//			else {
//				System.out.println("resource not found: "+"resources"+pathname);
//				continue;
//			}
//			try {
//				File f = new File(url.toURI());
////				File f = new File(url.toString());
////				System.out.println("bpy_import_meth: "+url.toString());
////				System.out.println("bpy_import_meth: "+f.getAbsolutePath());
//				list.clear();
...
//			}
//			
//			PyTuple found = (PyTuple)imp.find_module(modname, list);
//			PyModule mod = (PyModule)imp.load_module(modname, (PyFile)found.pyget(0), (PyString)found.pyget(1), (PyTuple)found.pyget(2));
//			for (PyObject key : mod.__dict__.asIterable()) {
//				PyObject py_class = mod.__getattr__(key.toString());
////				System.out.println("Found py attribute in "+modname+": "+key.toString());
//				if (py_class.__findattr__("__rna__") != null) {
////					System.out.println("found __rna__ in "+key.toString());
//					BpyRna.pyrna_basetype_register(null, py_class);
//				}
//			}
//			
//			((PyFile)found.pyget(0)).close();
//		}
//	}
	// TMP
	private static void bpy_import_meth(String pathname, String[] modnames) {
		for (String modname : modnames) {
			InputStream file = ResourceAnchor.class.getClassLoader().getResourceAsStream("resources"+pathname+modname+".py");
			if (file != null) {
//				System.out.println("resource found: "+"resources"+pathname+modname+".py");
				System.out.println("loading file: "+pathname+modname+".py");
			}
			else {
				System.out.println("resource not found: "+"resources"+pathname+modname+".py");
				continue;
			}
			try {
            	PyCode code = Py.compile(file, modname, CompileMode.exec);
            	file.close();
    			PyModule mod = (PyModule)imp.createFromCode(modname, code);
    			for (PyObject key : mod.__dict__.asIterable()) {
    				PyObject py_class = mod.__getattr__(key.toString());
//    				System.out.println("Found py attribute in "+modname+": "+key.toString());
    				if (py_class.__findattr__("__rna__") != null) {
