...
public class View3dSelect {


public static void view3d_set_viewcontext(bContext C, ViewContext vc)
{
...
//	vc.obact= bContext.CTX_data_active_object(C);
	vc.obedit= bContext.CTX_data_edit_object(C);
}
...
static short mixed_bones_object_selectbuffer(ViewContext vc, IntBuffer buffer, int[] mval)
{
	rcti rect = new rcti();
	int offs;
	...
	Rct.BLI_init_rcti(rect, mval[0]-14, mval[0]+14, mval[1]-14, mval[1]+14);
	hits15= View3dView.view3d_opengl_select(vc, buffer, View3dStruct.MAXPICKBUF, rect);
	if(hits15>0) {
		for(a=0; a<hits15; a++)
                    if((buffer.get(4*a+3) & 0xFFFF0000)!=0)
                        has_bones15= 1;

		offs= 4*hits15;
                buffer.position(offs);
		Rct.BLI_init_rcti(rect, mval[0]-9, mval[0]+9, mval[1]-9, mval[1]+9);
		hits9= View3dView.view3d_opengl_select(vc, buffer.slice(), View3dStruct.MAXPICKBUF-offs, rect);
		if(hits9>0) {
			for(a=0; a<hits9; a++)
                            if((buffer.get(offs+4*a+3) & 0xFFFF0000)!=0)
                                has_bones9= 1;

			offs+= 4*hits9;
                        buffer.position(offs);
			Rct.BLI_init_rcti(rect, mval[0]-5, mval[0]+5, mval[1]-5, mval[1]+5);
			hits5= View3dView.view3d_opengl_select(vc, buffer.slice(), View3dStruct.MAXPICKBUF-offs, rect);
			if(hits5>0) {
				for(a=0; a<hits5; a++)
                                    if((buffer.get(offs+4*a+3) & 0xFFFF0000)!=0)
                                        has_bones5= 1;
...
//			memcpy(buffer, buffer+offs, 4*offs);
                        for (int i=0; i<offs; i++) buffer.put(i, buffer.get(offs+i));
			return hits5;
		}
		if(has_bones9!=0) {
			offs= 4*hits15;
...
private static int[] lastmval={-100,-100};
/* mval is region coords */
static void mouse_select(bContext C, int[] mval, boolean extend, boolean obcenter)
{
...
//	unsigned int buffer[4*MAXPICKBUF];
   // IntBuffer buffer = BufferUtil.newIntBuffer(4*View3dStruct.MAXPICKBUF);
    IntBuffer buffer = Buffers.newDirectIntBuffer(4*View3dStruct.MAXPICKBUF);
	int temp, a, dist=100;
	short hits;
	...
	startbase=  (Base)scene.base.first;
	if(scene.basact!=null && scene.basact.next!=null)
            startbase= scene.basact.next;
...
//				static short lastmval[2]={-100, -100};
				...
				if(v3d.drawtype>ObjectTypes.OB_WIRE) {
					donearest= 1;
					if(UtilDefines.ABS(mval[0]-lastmval[0])<3 && UtilDefines.ABS(mval[1]-lastmval[1])<3) {
						if(has_bones==0)	/* hrms, if theres bones we always do nearest */
							donearest= 0;
					}
				}
				...
				if(donearest!=0) {
					...
					if(has_bones!=0) {
						...
						for(a=0; a<hits; a++) {
							if( min > (buffer.get(4*a+1)&0xFFFFFFFFL) && (buffer.get(4*a+3) & 0xFFFF0000)!=0 ) {
								min= buffer.get(4*a+1)&0xFFFFFFFFL;
								selcol= buffer.get(4*a+3) & 0xFFFF;
							}
						}
					}
					else {
						/* only exclude active object when it is selected... */
						if(scene.basact!=null && (scene.basact.flag & Blender.SELECT)!=0 && hits>1)
                                                    notcol= scene.basact.selcol;

						for(a=0; a<hits; a++) {
							if( min > (buffer.get(4*a+1)&0xFFFFFFFFL) && notcol!=(buffer.get(4*a+3) & 0xFFFF)) {
								min= buffer.get(4*a+1)&0xFFFFFFFFL;
								selcol= buffer.get(4*a+3) & 0xFFFF;
							}
						}
					...
					while(base!=null) {
						...
						if ((base.object.restrictflag & ObjectTypes.OB_RESTRICT_SELECT)!=0) {
							base=base.next;
							if(base==null) base= (Base)scene.base.first;
							if(base==startbase) break;
						}
						...
						if((base.lay & v3d.lay)!=0) {
							for(a=0; a<hits; a++) {
								if(has_bones!=0) {
									...
									if((buffer.get(4*a+3) & 0xFFFF0000)!=0) {
										if(base.selcol== (buffer.get((4*a)+3) & 0xFFFF))
											basact= base;
									}
								}
								else {
...
//				}
	...
	if(basact!=null) {
		...
		if(vc.obedit!=null) {
			...
			ObjectEdit.ED_base_object_select(basact, ObjectTypes.BA_SELECT);
		}
		/* also prevent making it active on mouse selection */
//		else if (BASE_SELECTABLE(v3d, basact)) {
                else if ((basact.lay & v3d.lay)!=0 && (basact.object.restrictflag & (ObjectTypes.OB_RESTRICT_SELECT|ObjectTypes.OB_RESTRICT_VIEW))==0) {

			oldbasact= scene.basact;
...
//			}
				...
				if((basact.flag & Blender.SELECT)!=0) {
					if(basact==oldbasact)
						ObjectEdit.ED_base_object_select(basact, ObjectTypes.BA_DESELECT);
				}
				else ObjectEdit.ED_base_object_select(basact, ObjectTypes.BA_SELECT);
			}

			if(oldbasact != basact) {
				ObjectEdit.ED_base_object_activate(C, basact); /* adds notifier */
			}
...
public static wmOperatorType.Operator view3d_select_invoke = new wmOperatorType.Operator() {
public int run(bContext C, wmOperator op, wmEvent event)
//static int view3d_select_invoke(bContext *C, wmOperator *op, wmEvent *event)
{
//        System.out.println("view3d_select_invoke");
...
//		PE_mouse_particles(C, event.mval, extend);
		...
		mouse_select(C, event.mval, extend, false);
        }

	/* allowing tweaks */
	return WindowManagerTypes.OPERATOR_PASS_THROUGH|WindowManagerTypes.OPERATOR_FINISHED;
}};

public static OpFunc VIEW3D_OT_select = new OpFunc() {
...
//void VIEW3D_OT_select(wmOperatorType *ot)
{
	...
	/* properties */
	RnaDefine.RNA_def_boolean(ot.srna, "extend", 0, "Extend", "Extend selection instead of deselecting everyting first.");
}};
...
//public static OpFunc VIEW3D_OT_drag = new OpFunc() {
//public void run(wmOperatorType ot)
