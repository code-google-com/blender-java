/**
 * $Id: ObjectEdit.java,v 1.2 2009/09/18 05:20:56 jladere Exp $
 *
 ...
 */
package blender.editors.object;
...
import blender.editors.space_view3d.View3dHeader;
...
public static final int EM_FREEDATA=		1;
public static final int EM_FREEUNDO=		2;
public static final int EM_WAITCURSOR=	4;
public static final int EM_DO_UNDO=		8;
...
public static void ED_base_object_select(Base base, int mode)
{
	if (base!=null) {
		if (mode==ObjectTypes.BA_SELECT) {
			if ((base.object.restrictflag & ObjectTypes.OB_RESTRICT_SELECT)==0)
				if (mode==ObjectTypes.BA_SELECT) base.flag |= Blender.SELECT;
		}
		else if (mode==ObjectTypes.BA_DESELECT) {
...
public static void ED_base_object_activate(bContext C, Base base)
{
	...
	if(base!=null && base.object.type!=ObjectTypes.OB_MESH)
	...
	if(base!=null) {
		...
		for(tbase=(Base)scene.base.first; tbase!=null; tbase= tbase.next) {
			if(base!=tbase && (tbase.object.shapeflag & ObjectTypes.OB_SHAPE_TEMPLOCK)!=0) {
				tbase.object.shapeflag &= ~ObjectTypes.OB_SHAPE_TEMPLOCK;
//				DAG_object_flush_update(scene, tbase.object, ObjectTypes.OB_RECALC_DATA);
			}
		}
		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_OB_ACTIVE, scene);
	}
	else
		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_OB_ACTIVE, null);
}
...
public static void ED_object_base_init_from_view(bContext C, Base base)
{
	...
	if (v3d==null) {
		base.lay = scene.lay;
		UtilDefines.VECCOPY(ob.loc, scene.cursor);
	}
	else {
//		if (v3d.localview!=0) {
		if (v3d.localvd!=null) {
			base.lay= ob.lay= v3d.layact | v3d.lay;
			UtilDefines.VECCOPY(ob.loc, v3d.cursor);
		}
		else {
		...
		if ((U.flag & UserDefTypes.USER_ADD_VIEWALIGNED)!=0) {
			ARegion ar= bContext.CTX_wm_region(C);
			if(ar!=null) {
				RegionView3D rv3d= (RegionView3D)ar.regiondata;

				rv3d.viewquat[0]= -rv3d.viewquat[0];
				Arithb.QuatToEul(rv3d.viewquat, ob.rot);
				rv3d.viewquat[0]= -rv3d.viewquat[0];
			}
		}
	}
	ObjectUtil.where_is_object(scene, ob);
}

/* ******************* add object operator ****************** */

public static EnumPropertyItem[] prop_object_types = {
	new EnumPropertyItem(ObjectTypes.OB_MESH, "MESH", 0, "Mesh", ""),
	new EnumPropertyItem(ObjectTypes.OB_CURVE, "CURVE", 0, "Curve", ""),
	new EnumPropertyItem(ObjectTypes.OB_SURF, "SURFACE", 0, "Surface", ""),
	new EnumPropertyItem(ObjectTypes.OB_MBALL, "META", 0, "Meta", ""),
	new EnumPropertyItem(ObjectTypes.OB_FONT, "TEXT", 0, "Text", ""),
	new EnumPropertyItem(0, "", 0, null, null),
	new EnumPropertyItem(ObjectTypes.OB_ARMATURE, "ARMATURE", 0, "Armature", ""),
	new EnumPropertyItem(ObjectTypes.OB_LATTICE, "LATTICE", 0, "Lattice", ""),
	new EnumPropertyItem(ObjectTypes.OB_EMPTY, "EMPTY", 0, "Empty", ""),
	new EnumPropertyItem(0, "", 0, null, null),
	new EnumPropertyItem(ObjectTypes.OB_CAMERA, "CAMERA", 0, "Camera", ""),
	new EnumPropertyItem(ObjectTypes.OB_LAMP, "LAMP", 0, "Lamp", ""),
...
public static bObject object_add_type(bContext C, int type)
{
	...
	if (bContext.CTX_data_edit_object(C)!=null)
		ED_object_exit_editmode(C, EM_FREEDATA|EM_FREEUNDO|EM_WAITCURSOR); /* freedata, and undo */

	/* deselects all, sets scene.basact */
	ob= ObjectUtil.add_object(scene, type);
	/* editor level activate, notifiers */
	ED_base_object_activate(C, scene.basact);

	/* more editor stuff */
	ED_object_base_init_from_view(C, scene.basact);
...
public static EnumPropertyItem[] prop_mesh_types = {
	new EnumPropertyItem(0, "PLANE", 0/*ICON_MESH_PLANE*/, "Plane", ""),
	new EnumPropertyItem(1, "CUBE", 0/*ICON_MESH_CUBE*/, "Cube", ""),
	new EnumPropertyItem(2, "CIRCLE", 0/*ICON_MESH_CIRCLE*/, "Circle", ""),
	new EnumPropertyItem(3, "UVSPHERE", 0/*ICON_MESH_UVSPHERE*/, "UVsphere", ""),
	new EnumPropertyItem(4, "ICOSPHERE", 0/*ICON_MESH_ICOSPHERE*/, "Icosphere", ""),
	new EnumPropertyItem(5, "CYLINDER", 0/*ICON_MESH_TUBE*/, "Cylinder", ""),
	new EnumPropertyItem(6, "CONE", 0/*ICON_MESH_CONE*/, "Cone", ""),
	new EnumPropertyItem(0, "", 0, null, null),
	new EnumPropertyItem(7, "GRID", 0/*ICON_MESH_GRID*/, "Grid", ""),
	new EnumPropertyItem(8, "MONKEY", 0/*ICON_MESH_MONKEY*/, "Monkey", ""),
	new EnumPropertyItem(0, null, 0, null, null)
};

public static wmOperatorType.Operator object_add_mesh_exec = new wmOperatorType.Operator() {
...
//static int object_add_mesh_exec(bContext *C, wmOperator *op)
{
	...
	if(obedit==null || obedit.type!=ObjectTypes.OB_MESH) {
		object_add_type(C, ObjectTypes.OB_MESH);
		ED_object_enter_editmode(C, EM_DO_UNDO);
		newob = 1;
	}
//	else DAG_object_flush_update(bContext.CTX_data_scene(C), obedit, ObjectTypes.OB_RECALC_DATA);

	switch(RnaAccess.RNA_enum_get(op.ptr, "type")) {
		case 0:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_plane_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
		case 1:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_cube_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
		case 2:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_circle_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
		case 3:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_uv_sphere_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
		case 4:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_ico_sphere_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
		case 5:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_cylinder_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
		case 6:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_cone_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
		case 7:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_grid_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
		case 8:
			WmEventSystem.WM_operator_name_call(C, "MESH_OT_primitive_monkey_add", WmTypes.WM_OP_INVOKE_REGION_WIN, null);
			break;
	}
	/* userdef */
	if (newob!=0 && (U.flag & UserDefTypes.USER_ADD_EDITMODE)==0) {
		ED_object_exit_editmode(C, EM_FREEDATA);
	}

	WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_OBJECT|WmTypes.ND_GEOM_SELECT, obedit);

	return WindowManagerTypes.OPERATOR_FINISHED;
}};


public static OpFunc OBJECT_OT_mesh_add = new OpFunc() {
...
//void OBJECT_OT_mesh_add(wmOperatorType *ot)
{
	...
	RnaDefine.RNA_def_enum(ot.srna, "type", prop_mesh_types, 0, "Primitive", "");
}};
...
//};
//
public static wmOperatorType.Operator object_add_curve_exec = new wmOperatorType.Operator() {
public int run(bContext C, wmOperator op, wmEvent event)
...
public static wmOperatorType.Operator object_add_curve_invoke = new wmOperatorType.Operator() {
public int run(bContext C, wmOperator op, wmEvent event)
...
public static OpFunc OBJECT_OT_curve_add = new OpFunc() {
public void run(wmOperatorType ot)
//void OBJECT_OT_curve_add(wmOperatorType *ot)
{
	...
	/* flags */
	ot.flag= WmTypes.OPTYPE_REGISTER|WmTypes.OPTYPE_UNDO;

//	RNA_def_enum(ot.srna, "type", prop_curve_types, 0, "Primitive", "");
}};

//public static EnumPropertyItem[] prop_surface_types= {
//	new EnumPropertyItem(CU_PRIM_CURVE|CU_NURBS, "NURBS_CURVE", 0/*ICON_SURFACE_NCURVE*/, "NURBS Curve", ""),
//	new EnumPropertyItem(CU_PRIM_CIRCLE|CU_NURBS, "NURBS_CIRCLE", 0/*ICON_SURFACE_NCIRCLE*/, "NURBS Circle", ""),
//	new EnumPropertyItem(CU_PRIM_PATCH|CU_NURBS, "NURBS_SURFACE", 0/*ICON_SURFACE_NSURFACE*/, "NURBS Surface", ""),
//	new EnumPropertyItem(CU_PRIM_TUBE|CU_NURBS, "NURBS_TUBE", 0/*ICON_SURFACE_NTUBE*/, "NURBS Tube", ""),
//	new EnumPropertyItem(CU_PRIM_SPHERE|CU_NURBS, "NURBS_SPHERE", 0/*ICON_SURFACE_NSPHERE*/, "NURBS Sphere", ""),
//	new EnumPropertyItem(CU_PRIM_DONUT|CU_NURBS, "NURBS_DONUT", 0/*ICON_SURFACE_NDONUT*/, "NURBS Donut", ""),
//	new EnumPropertyItem(0, null, 0, null, null)
//};

public static wmOperatorType.Operator object_add_surface_exec = new wmOperatorType.Operator() {
public int run(bContext C, wmOperator op, wmEvent event)
...
public static OpFunc OBJECT_OT_surface_add = new OpFunc() {
public void run(wmOperatorType ot)
...
public static wmOperatorType.Operator object_add_text_exec = new wmOperatorType.Operator() {
public int run(bContext C, wmOperator op, wmEvent event)
...
public static OpFunc OBJECT_OT_text_add = new OpFunc() {
public void run(wmOperatorType ot)
...
public static wmOperatorType.Operator object_armature_add_exec = new wmOperatorType.Operator() {
public int run(bContext C, wmOperator op, wmEvent event)
...
public static OpFunc OBJECT_OT_armature_add = new OpFunc() {
public void run(wmOperatorType ot)
...
//	if(flag & EM_WAITCURSOR) waitcursor(1);
	if(obedit.type==ObjectTypes.OB_MESH) {
		...
		if(((EditMesh)me.edit_mesh).totvert>MeshTypes.MESH_MAX_VERTS) {
//			error("Too many vertices");
                        System.err.println("Too many vertices");
			return;
		}
		EditMeshUtil.load_editMesh(scene, obedit);

		if(freedata!=0) {
			EditMeshUtil.free_editMesh((EditMesh)me.edit_mesh);
...
//		if(flag & EM_WAITCURSOR) waitcursor(0);
		...
		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_MODE|WmTypes.NS_MODE_OBJECT, scene);
	}
}
...
public static void ED_object_enter_editmode(bContext C, int flag)
{
	...
	if(scene.id.lib!=null) return;
	if(base==null) return;

	if(sa!=null && sa.spacetype==SpaceTypes.SPACE_VIEW3D)
		v3d= (View3D)sa.spacedata.first;

	if(v3d!=null && (base.lay & v3d.lay)==0) return;
	else if(v3d==null && (base.lay & scene.lay)==0) return;
	...
	if (ObjectUtil.object_data_is_libdata(ob)) {
//		error_libdata();
                System.err.println("error_libdata");
		return;
	}
...
//		if(me.pv) mesh_pmv_off(ob, me);
		...
		EditMeshUtil.make_editMesh(scene, ob);

		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_MODE|WmTypes.NS_EDITMODE_MESH, scene);
	}
	else if (ob.type==ObjectTypes.OB_ARMATURE){
		bArmature arm= (bArmature)base.object.data;
		if (arm==null) return;
		/*
...
//		DAG_object_flush_update(scene, ob, OB_RECALC);
		...
		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_MODE|WmTypes.NS_EDITMODE_ARMATURE, scene);
	}
	else if(ob.type==ObjectTypes.OB_FONT) {
		scene.obedit= ob; // XXX for context
		ok= 1;
// 		make_editText(ob);
		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_MODE|WmTypes.NS_EDITMODE_TEXT, scene);
	}
	else if(ob.type==ObjectTypes.OB_MBALL) {
		scene.obedit= ob; // XXX for context
		ok= 1;
//// XXX		make_editMball();
		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_MODE|WmTypes.NS_EDITMODE_MBALL, scene);

	}
	else if(ob.type==ObjectTypes.OB_LATTICE) {
		scene.obedit= ob; // XXX for context
		ok= 1;
//		make_editLatt(ob);

		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_MODE|WmTypes.NS_EDITMODE_LATTICE, scene);
	}
	else if(ob.type==ObjectTypes.OB_SURF || ob.type==ObjectTypes.OB_CURVE) {
		ok= 1;
		scene.obedit= ob; // XXX for context
//		make_editNurb(ob);

		WmEventSystem.WM_event_add_notifier(C, WmTypes.NC_SCENE|WmTypes.ND_MODE|WmTypes.NS_EDITMODE_CURVE, scene);
	}
