...
*/
public class EditMeshLib {

/* ****************** stats *************** */

public static int EM_nfaces_selected(EditMesh em)
{
	EditFace efa;
	int count= 0;
...
public static int EM_nedges_selected(EditMesh em)
{
	EditEdge eed;
	int count= 0;
...
public static int EM_nvertices_selected(EditMesh em)
{
	EditVert eve;
	int count= 0;
...
public static void EM_stats_update(EditMesh em)
{
...
public static void EM_set_actFace(EditMesh em, EditFace efa)
{
	em.act_face = efa;
}

public static EditFace EM_get_actFace(EditMesh em, boolean sloppy)
{
	if (em.act_face!=null) {
		return em.act_face;
	} else if (sloppy) {
		...
		for (; ese!=null; ese=ese.prev){
			if(ese.type == EditVertUtil.EDITFACE) {
...
public static boolean EM_get_actSelection(EditMesh em, EditSelection ese)
{
	EditSelection ese_last = em.selected.last;
	EditFace efa = EM_get_actFace(em, false);

	ese.next = ese.prev = null;

	if (ese_last!=null) {
		if (ese_last.type == EditVertUtil.EDITFACE) { /* if there is an active face, use it over the last selected face */
			if (efa!=null) {
				ese.data = (Object)efa;
			} else {
				ese.data = ese_last.data;
			}
			ese.type = EditVertUtil.EDITFACE;
		} else {
			ese.data = ese_last.data;
...
public static boolean EM_check_selection(EditMesh em, Object data)
{
	EditSelection ese;

	for(ese = em.selected.first; ese!=null; ese = ese.next){
		if(ese.data == data) return true;
	}

	return false;
}

public static void EM_remove_selection(EditMesh em, Object data, int type)
{
	EditSelection ese;
	for(ese=em.selected.first; ese!=null; ese = ese.next){
		if(ese.data == data){
			ListBaseUtil.BLI_freelinkN(em.selected, ese);
			break;
		}
	}
}

public static void EM_store_selection(EditMesh em, Object data, int type)
{
	EditSelection ese;
	if(!EM_check_selection(em, data)){
		ese = new EditSelection();
		ese.type = (short)type;
		ese.data = data;
		ListBaseUtil.BLI_addtail(em.selected, ese);
	}
}

public static void EM_validate_selections(EditMesh em)
{
	...
	while(ese!=null){
		nextese = ese.next;
		if(ese.type == EditVertUtil.EDITVERT && (((EditVert)ese.data).f & Blender.SELECT)==0)
                    ListBaseUtil.BLI_freelinkN(em.selected, ese);
		else if(ese.type == EditVertUtil.EDITEDGE && (((EditEdge)ese.data).f & Blender.SELECT)==0)
                    ListBaseUtil.BLI_freelinkN(em.selected, ese);
		else if(ese.type == EditVertUtil.EDITFACE && (((EditFace)ese.data).f & Blender.SELECT)==0)
                    ListBaseUtil.BLI_freelinkN(em.selected, ese);
		ese = nextese;
	}
}

public static void EM_strip_selections(EditMesh em)
{
	EditSelection ese, nextese;
	if((em.selectmode & SceneTypes.SCE_SELECT_VERTEX)==0){
		ese = em.selected.first;
		while(ese!=null){
			nextese = ese.next;
	...
	if((em.selectmode & SceneTypes.SCE_SELECT_EDGE)==0){
		ese=em.selected.first;
		while(ese!=null){
			nextese = ese.next;
			if(ese.type == EditVertUtil.EDITEDGE)
                            ListBaseUtil.BLI_freelinkN(em.selected, ese);
			ese = nextese;
		}
	}
	if((em.selectmode & SceneTypes.SCE_SELECT_FACE)==0){
		ese=em.selected.first;
		while(ese!=null){
			nextese = ese.next;
			if(ese.type == EditVertUtil.EDITFACE)
...
*/
public static void EM_editselection_center(float[] center, EditSelection ese)
{
	if (ese.type==EditVertUtil.EDITVERT) {
		EditVert eve= (EditVert)ese.data;
		Arithb.VecCopyf(center, eve.co);
	} else if (ese.type==EditVertUtil.EDITEDGE) {
		EditEdge eed= (EditEdge)ese.data;
		Arithb.VecAddf(center, eed.v1.co, eed.v2.co);
		Arithb.VecMulf(center, 0.5f);
	} else if (ese.type==EditVertUtil.EDITFACE) {
		EditFace efa= (EditFace)ese.data;
		Arithb.VecCopyf(center, efa.cent);
	}
}

public static void EM_editselection_normal(float[] normal, EditSelection ese)
{
	if (ese.type==EditVertUtil.EDITVERT) {
		EditVert eve= (EditVert)ese.data;
		Arithb.VecCopyf(normal, eve.no);
	} else if (ese.type==EditVertUtil.EDITEDGE) {
		EditEdge eed= (EditEdge)ese.data;
		float[] plane=new float[3]; /* need a plane to correct the normal */
		float[] vec=new float[3]; /* temp vec storage */

		Arithb.VecAddf(normal, eed.v1.no, eed.v2.no);
		...
		Arithb.Crossf(vec, normal, plane);
		Arithb.Crossf(normal, plane, vec);
		Arithb.Normalize(normal);

	} else if (ese.type==EditVertUtil.EDITFACE) {
		EditFace efa= (EditFace)ese.data;
		Arithb.VecCopyf(normal, efa.n);
	}
}
    ...
    public static void EM_editselection_plane(float[] plane, EditSelection ese)
    {
	if (ese.type==EditVertUtil.EDITVERT) {
		EditVert eve= (EditVert)ese.data;
		float[] vec={0,0,0};

		if (ese.prev!=null) { /*use previously selected data to make a usefull vertex plane */
			EM_editselection_center(vec, ese.prev);
			Arithb.VecSubf(plane, vec, eve.co);
		} else {
			...
			if (eve.no[0]<0.5f)
                ...
                vec[2]=1;
			Arithb.Crossf(plane, eve.no, vec);
		}
	} else if (ese.type==EditVertUtil.EDITEDGE) {
		EditEdge eed= (EditEdge)ese.data;
		...
		if (eed.v2.co[1] > eed.v1.co[1]) /*check which to do first */
			...
			Arithb.VecSubf(plane, eed.v1.co, eed.v2.co);

	} else if (ese.type==EditVertUtil.EDITFACE) {
		EditFace efa= (EditFace)ese.data;
		float[] vec=new float[3];
		if (efa.v4!=null) { /*if its a quad- set the plane along the 2 longest edges.*/
			float[] vecA=new float[3], vecB=new float[3];
			Arithb.VecSubf(vecA, efa.v4.co, efa.v3.co);
			Arithb.VecSubf(vecB, efa.v1.co, efa.v2.co);
			Arithb.VecAddf(plane, vecA, vecB);

			Arithb.VecSubf(vecA, efa.v1.co, efa.v4.co);
			Arithb.VecSubf(vecB, efa.v2.co, efa.v3.co);
			Arithb.VecAddf(vec, vecA, vecB);
			/*use the biggest edge length*/
			if (plane[0]*plane[0]+plane[1]*plane[1]+plane[2]*plane[2] < vec[0]*vec[0]+vec[1]*vec[1]+vec[2]*vec[2])
				Arithb.VecCopyf(plane, vec);
		} else {
			/*start with v1-2 */
			Arithb.VecSubf(plane, efa.v1.co, efa.v2.co);

			/*test the edge between v2-3, use if longer */
			Arithb.VecSubf(vec, efa.v2.co, efa.v3.co);
			if (plane[0]*plane[0]+plane[1]*plane[1]+plane[2]*plane[2] < vec[0]*vec[0]+vec[1]*vec[1]+vec[2]*vec[2])
				Arithb.VecCopyf(plane, vec);

			/*test the edge between v1-3, use if longer */
			Arithb.VecSubf(vec, efa.v3.co, efa.v1.co);
			if (plane[0]*plane[0]+plane[1]*plane[1]+plane[2]*plane[2] < vec[0]*vec[0]+vec[1]*vec[1]+vec[2]*vec[2])
				Arithb.VecCopyf(plane, vec);
		}
	}
...
public static void EM_select_face(EditFace efa, boolean sel)
{
	if(sel) {
		efa.f |= Blender.SELECT;
		efa.e1.f |= Blender.SELECT;
		efa.e2.f |= Blender.SELECT;
		efa.e3.f |= Blender.SELECT;
		if(efa.e4!=null) efa.e4.f |= Blender.SELECT;
		efa.v1.f |= Blender.SELECT;
		efa.v2.f |= Blender.SELECT;
		efa.v3.f |= Blender.SELECT;
		if(efa.v4!=null) efa.v4.f |= Blender.SELECT;
	}
	else {
		efa.f &= ~Blender.SELECT;
		efa.e1.f &= ~Blender.SELECT;
...
public static void EM_select_edge(EditEdge eed, boolean sel)
{
	if(sel) {
		eed.f |= Blender.SELECT;
		eed.v1.f |= Blender.SELECT;
		eed.v2.f |= Blender.SELECT;
	}
	else {
		eed.f &= ~Blender.SELECT;
		eed.v1.f &= ~Blender.SELECT;
...
public static void EM_select_face_fgon(EditMesh em, EditFace efa, boolean val)
{
	short index=0;

	if(efa.fgonf==0) EM_select_face(efa, val);
	else {
		if(efa.e1.fgoni!=0) index= efa.e1.fgoni;
		if(efa.e2.fgoni!=0) index= efa.e2.fgoni;
		if(efa.e3.fgoni!=0) index= efa.e3.fgoni;
		if(efa.v4!=null && efa.e4.fgoni!=0) index= efa.e4.fgoni;

		if(index==0) System.out.printf("wrong fgon select\n");

		// select all ngon faces with index
		for(efa= em.faces.first; efa!=null; efa= efa.next) {
			if(efa.fgonf!=0) {
				if(efa.e1.fgoni==index || efa.e2.fgoni==index ||
				   efa.e3.fgoni==index || (efa.e4!=null && efa.e4.fgoni==index) ) {
...
/* only vertices */
public static boolean faceselectedOR(EditFace efa, int flag)
{
	if (((efa.v1.f | efa.v2.f | efa.v3.f | (efa.v4!=null?efa.v4.f:0))&flag)!=0) {
		return true;
	} else {
		return false;
	}
...
public static void EM_clear_flag_all(EditMesh em, int flag)
{
	EditVert eve;
	EditEdge eed;
	EditFace efa;

	for (eve= em.verts.first; eve!=null; eve= eve.next) eve.f &= ~flag;
	for (eed= em.edges.first; eed!=null; eed= eed.next) eed.f &= ~flag;
	for (efa= em.faces.first; efa!=null; efa= efa.next) efa.f &= ~flag;

	if((flag & Blender.SELECT)!=0) {
		ListBaseUtil.BLI_freelistN(em.selected);
...
public static void EM_select_flush(EditMesh em)
{
	EditEdge eed;
	EditFace efa;

	for(eed= em.edges.first; eed!=null; eed= eed.next) {
		if((eed.v1.f & eed.v2.f & Blender.SELECT)!=0)
                    eed.f |= Blender.SELECT;
	}
	for(efa= em.faces.first; efa!=null; efa= efa.next) {
		if(efa.v4!=null) {
			if((efa.v1.f & efa.v2.f & efa.v3.f & efa.v4.f & Blender.SELECT )!=0)
                            efa.f |= Blender.SELECT;
		}
		else {
			if((efa.v1.f & efa.v2.f & efa.v3.f & Blender.SELECT )!=0)
                            efa.f |= Blender.SELECT;
		}
	}
	EM_nedges_selected(em);
...
public static void check_fgons_selection(EditMesh em)
{
	EditFace efa, efan;
	EditEdge eed;
	ListBase<EditFace>[] lbar;
	...
	lbar= new ListBase[totfgon+1];

    /* put all fgons in lbar */
	for(efa= em.faces.first; efa!=null; efa= efan) {
		efan= efa.next;
		index= efa.e1.fgoni;
		if(index==0) index= efa.e2.fgoni;
		if(index==0) index= efa.e3.fgoni;
		if(index==0 && efa.e4!=null) index= efa.e4.fgoni;
		if(index!=0) {
			ListBaseUtil.BLI_remlink(em.faces, efa);
                        lbar[index] = new ListBase<EditFace>();
			ListBaseUtil.BLI_addtail(lbar[index], efa);
		}
	}
	...
	for(index=1; index<=totfgon; index++) {
		...
		sel= desel= 0;
		for(efa= lbar[index].first; efa!=null; efa= efa.next) {
			if(efa.e1.fgoni==0) {
				if((efa.e1.f & Blender.SELECT)!=0) sel++;
				else desel++;
			}
			if(efa.e2.fgoni==0) {
				if((efa.e2.f & Blender.SELECT)!=0) sel++;
				else desel++;
			}
			if(efa.e3.fgoni==0) {
				if((efa.e3.f & Blender.SELECT)!=0) sel++;
				else desel++;
			}
			if(efa.e4!=null && efa.e4.fgoni==0) {
				if((efa.e4.f & Blender.SELECT)!=0) sel++;
				else desel++;
			}

			if(sel!=0 && desel!=0) break;
		}

		if(sel!=0 && desel!=0) sel= 0;
		else if(sel!=0) sel= 1;
		else sel= 0;

		/* select/deselect and put back */
		for(efa= lbar[index].first; efa!=null; efa= efa.next) {
			if(sel!=0) efa.f |= Blender.SELECT;
			else efa.f &= ~Blender.SELECT;
		}
		ListBaseUtil.addlisttolist(em.faces, lbar[index]);
	}
}
...
public static void EM_selectmode_flush(EditMesh em)
{
	EditEdge eed;
	EditFace efa;
	...
	if((em.selectmode & SceneTypes.SCE_SELECT_VERTEX)!=0) {
		for(eed= em.edges.first; eed!=null; eed= eed.next) {
			if((eed.v1.f & eed.v2.f & Blender.SELECT)!=0) eed.f |= Blender.SELECT;
			else eed.f &= ~Blender.SELECT;
		}
		for(efa= em.faces.first; efa!=null; efa= efa.next) {
			if(efa.v4!=null) {
				if((efa.v1.f & efa.v2.f & efa.v3.f & efa.v4.f & Blender.SELECT)!=0) efa.f |= Blender.SELECT;
				else efa.f &= ~Blender.SELECT;
			}
			else {
				if((efa.v1.f & efa.v2.f & efa.v3.f & Blender.SELECT)!=0) efa.f |= Blender.SELECT;
				else efa.f &= ~Blender.SELECT;
			}
		}
	}
	// flush to faces
	else if((em.selectmode & SceneTypes.SCE_SELECT_EDGE)!=0) {
		for(efa= em.faces.first; efa!=null; efa= efa.next) {
			if(efa.e4!=null) {
				if((efa.e1.f & efa.e2.f & efa.e3.f & efa.e4.f & Blender.SELECT)!=0) efa.f |= Blender.SELECT;
				else efa.f &= ~Blender.SELECT;
			}
			else {
				if((efa.e1.f & efa.e2.f & efa.e3.f & Blender.SELECT)!=0) efa.f |= Blender.SELECT;
				else efa.f &= ~Blender.SELECT;
			}
		}
	}
	// make sure selected faces have selected edges too, for extrude (hack?)
	else if((em.selectmode & SceneTypes.SCE_SELECT_FACE)!=0) {
		for(efa= em.faces.first; efa!=null; efa= efa.next) {
			if((efa.f & Blender.SELECT)!=0) EM_select_face(efa, true);
		}
	}
...
public static void EM_selectmode_set(EditMesh em)
{
	EditVert eve;
	EditEdge eed;
	EditFace efa;
	...
	if((em.selectmode & SceneTypes.SCE_SELECT_VERTEX)!=0) {
		...
		for (eed= em.edges.first; eed!=null; eed= eed.next) eed.f &= ~Blender.SELECT;
		for (efa= em.faces.first; efa!=null; efa= efa.next) efa.f &= ~Blender.SELECT;

		EM_select_flush(em);
	}
	else if((em.selectmode & SceneTypes.SCE_SELECT_EDGE)!=0) {
		/* deselect vertices, and select again based on edge select */
		for(eve= em.verts.first; eve!=null; eve= eve.next) eve.f &= ~Blender.SELECT;
		for(eed= em.edges.first; eed!=null; eed= eed.next)
			if((eed.f & Blender.SELECT)!=0) EM_select_edge(eed, true);
		/* selects faces based on edge status */
		EM_selectmode_flush(em);
	}
	else if((em.selectmode & SceneTypes.SCE_SELECT_FACE)!=0) {
		/* deselect eges, and select again based on face select */
		for(eed= em.edges.first; eed!=null; eed= eed.next) EM_select_edge(eed, false);
...
*/
public static void EM_hide_reset(EditMesh em)
{
	EditEdge eed;
	EditFace efa;

	for(eed= em.edges.first; eed!=null; eed= eed.next)
		if(eed.v1.h!=0 || eed.v2.h!=0) eed.h |= 1;

	for(efa= em.faces.first; efa!=null; efa= efa.next)
		if((efa.e1.h & 1)!=0 || (efa.e2.h & 1)!=0 || (efa.e3.h & 1)!=0 || (efa.e4!=null && (efa.e4.h & 1)!=0))
			efa.h= 1;
...
public static void recalc_editnormals(EditMesh em)
{
	EditFace efa;
	EditVert eve;

	for(eve= em.verts.first; eve!=null; eve=eve.next) {
		eve.no[0] = eve.no[1] = eve.no[2] = 0.0f;
	}
	...
	for(efa= em.faces.first; efa!=null; efa=efa.next) {
		if(efa.v4!=null) {
			...
			Arithb.VecAddf(efa.v4.no, efa.v4.no, efa.n);
		}
		else {
			Arithb.CalcNormFloat(efa.v1.co, efa.v2.co, efa.v3.co, efa.n);
			Arithb.CalcCent3f(efa.cent, efa.v1.co, efa.v2.co, efa.v3.co);
		}
		Arithb.VecAddf(efa.v1.no, efa.v1.no, efa.n);
		Arithb.VecAddf(efa.v2.no, efa.v2.no, efa.n);
		Arithb.VecAddf(efa.v3.no, efa.v3.no, efa.n);
	}

	/* following Mesh convention; we use vertex coordinate itself for normal in this case */
	for(eve= em.verts.first; eve!=null; eve=eve.next) {
		if (Arithb.Normalize(eve.no)==0.0) {
			UtilDefines.VECCOPY(eve.no, eve.co);
			Arithb.Normalize(eve.no);
		}
	}
}
...
public static float EM_face_area(EditFace efa)
{
	if(efa.v4!=null) return Arithb.AreaQ3Dfl(efa.v1.co, efa.v2.co, efa.v3.co, efa.v4.co);
	else return Arithb.AreaT3Dfl(efa.v1.co, efa.v2.co, efa.v3.co);
}
...
public static void EM_fgon_flags(EditMesh em)
{
	EditFace efa, efan, efamax;
	EditEdge eed;
	ListBase<EditFace> listb=new ListBase<EditFace>();
	float size, maxsize;
	short done, curindex= 1;

	// for each face with fgon edge AND not fgon flag set
	for(eed= em.edges.first; eed!=null; eed= eed.next) eed.fgoni= 0;  // index
	for(efa= em.faces.first; efa!=null; efa= efa.next) efa.fgonf= 0;  // flag

	// for speed & simplicity, put fgon face candidates in new listbase
	efa= em.faces.first;
	while(efa!=null) {
		efan= efa.next;
		if( (efa.e1.h & EditMeshUtil.EM_FGON)!=0 || (efa.e2.h & EditMeshUtil.EM_FGON)!=0 ||
			(efa.e3.h & EditMeshUtil.EM_FGON)!=0 || (efa.e4!=null && (efa.e4.h & EditMeshUtil.EM_FGON)!=0) ) {
			ListBaseUtil.BLI_remlink(em.faces, efa);
			ListBaseUtil.BLI_addtail(listb, efa);
		}
		efa= efan;
	}

	// find an undone face with fgon edge
	for(efa= listb.first; efa!=null; efa= efa.next) {
		if(efa.fgonf==0) {

			// init this face
			efa.fgonf= EditMeshUtil.EM_FGON;
			if((efa.e1.h & EditMeshUtil.EM_FGON)!=0) efa.e1.fgoni= curindex;
			if((efa.e2.h & EditMeshUtil.EM_FGON)!=0) efa.e2.fgoni= curindex;
			if((efa.e3.h & EditMeshUtil.EM_FGON)!=0) efa.e3.fgoni= curindex;
			if(efa.e4!=null && (efa.e4.h & EditMeshUtil.EM_FGON)!=0) efa.e4.fgoni= curindex;
			...
			while(done==1) {
				...
				for(efan= listb.first; efan!=null; efan= efan.next) {
					if(efan.fgonf==0) {
						// if one if its edges has index set, do other too
						if( (efan.e1.fgoni==curindex) || (efan.e2.fgoni==curindex) ||
							(efan.e3.fgoni==curindex) || (efan.e4!=null && (efan.e4.fgoni==curindex)) ) {

							efan.fgonf= EditMeshUtil.EM_FGON;
							if((efan.e1.h & EditMeshUtil.EM_FGON)!=0) efan.e1.fgoni= curindex;
							if((efan.e2.h & EditMeshUtil.EM_FGON)!=0) efan.e2.fgoni= curindex;
							if((efan.e3.h & EditMeshUtil.EM_FGON)!=0) efan.e3.fgoni= curindex;
							if(efan.e4!=null && (efan.e4.h & EditMeshUtil.EM_FGON)!=0) efan.e4.fgoni= curindex;
			...
			efamax.fgonf |= EditMeshUtil.EM_FGON_DRAW;
			curindex++;
	...
	while(efa!=null) {
		efan= efa.next;
		ListBaseUtil.BLI_remlink(listb, efa);
	...
	for(eed= em.edges.first; eed!=null; eed= eed.next) {
		if(eed.fgoni==0) eed.h &= ~EditMeshUtil.EM_FGON;
	}
