...
//	}
	...
	public static wmOperatorType.Operator screen_opengl_render_cancel = new wmOperatorType.Operator() {
...
//	}
	...
	public static wmOperatorType.Operator screen_opengl_render_modal = new wmOperatorType.Operator() {
	public int run(bContext C, wmOperator op, wmEvent event)
	{
//		OGLRender *oglrender= op->customdata;
...
//		}
	...
	public static wmOperatorType.Operator screen_opengl_render_invoke = new wmOperatorType.Operator() {
	public int run(bContext C, wmOperator op, wmEvent event)
	{
//		OGLRender *oglrender;
//		int anim= RNA_boolean_get(op->ptr, "animation");
...
//		oglrender->timer= WM_event_add_timer(CTX_wm_manager(C), CTX_wm_window(C), TIMER, 0.01f);
	...
	public static wmOperatorType.Operator screen_opengl_render_exec = new wmOperatorType.Operator() {
	public int run(bContext C, wmOperator op, wmEvent event)
	{
//		const short is_animation= RNA_boolean_get(op->ptr, "animation");
//
...
//		WM_event_add_notifier(C, NC_SCENE|ND_RENDER_RESULT, CTX_data_scene(C));
	...
	public static OpFunc RENDER_OT_opengl = new OpFunc() {
	public void run(wmOperatorType ot)
	{
		...
		ot.poll= ScreenOps.ED_operator_screenactive;

		RnaDefine.RNA_def_boolean(ot.srna, "animation", 0, "Animation", "Render files from the animation range of this scene");
		RnaDefine.RNA_def_boolean(ot.srna, "write_still", 0, "Write Image", "Save rendered the image to the output path (used only when animation is disabled)");
		RnaDefine.RNA_def_boolean(ot.srna, "view_context", 1, "View Context", "Use the current 3D view for rendering, else use scene settings.");
	}};
