...
//	/**
//	 * $Id: view2d_ops.c 34179 2011-01-08 18:37:11Z ton $
//	 *
...
//	#include "PIL_time.h" /* USER_ZOOM_CONT */
	...
	public static wmOperatorType.Operator view2d_poll = new wmOperatorType.Operator() {
	public int run(bContext C, wmOperator op, wmEvent event)
	{
		ARegion ar= bContext.CTX_wm_region(C);

		return ((ar != null) && (ar.v2d.flag & View2dTypes.V2D_IS_INITIALISED)!=0)?1:0;
	}};
...
//	}
	...
	public static OpFunc VIEW2D_OT_pan = new OpFunc() {
...
//	}
	...
	public static OpFunc VIEW2D_OT_scroll_right = new OpFunc() {
	public void run(wmOperatorType ot)
	{
//		/* identifiers */
...
//	}
	...
	public static OpFunc VIEW2D_OT_scroll_left = new OpFunc() {
	public void run(wmOperatorType ot)
	{
//		/* identifiers */
//		ot->name= "Scroll Left";
...
//	}
	...
	public static OpFunc VIEW2D_OT_scroll_down = new OpFunc() {
	public void run(wmOperatorType ot)
	{
//		/* identifiers */
//		ot->name= "Scroll Down";
...
//	}
	...
	public static OpFunc VIEW2D_OT_scroll_up = new OpFunc() {
	public void run(wmOperatorType ot)
	{
//		/* identifiers */
//		ot->name= "Scroll Up";
...
//	}
	...
	public static class v2dScrollerMove {
		public View2D v2d;			/* View2D data that this operation affects */
		public ARegion ar;			/* region that the scroller is in */
		
		public short scroller;			/* scroller that mouse is in ('h' or 'v') */
		public short zone;				/* -1 is min zoomer, 0 is bar, 1 is max zoomer */ // XXX find some way to provide visual feedback of this (active color?)
		
		public float fac;				/* view adjustment factor, based on size of region */
		public float delta;			/* amount moved by mouse on axis of interest */
		
		public float scrollbarwidth;	/* width of the scrollbar itself, used for page up/down clicks */
		
		public int lastx, lasty;		/* previous mouse coordinates (in screen coordinates) for determining movement */
	};
...
//	enum {
		public static final int SCROLLHANDLE_MIN= -1;
		public static final int SCROLLHANDLE_BAR= 0;
		public static final int SCROLLHANDLE_MAX= 1;
		public static final int SCROLLHANDLE_MIN_OUTSIDE= 2;
		public static final int SCROLLHANDLE_MAX_OUTSIDE= 3;
//	} eV2DScrollerHandle_Zone;
	...
	{
		short in_view=1;
		boolean in_min, in_max, in_bar, out_min, out_max;
		...
		in_max= ( (mouse >= (sh_max - View2dUtil.V2D_SCROLLER_HANDLE_SIZE)) && (mouse <= (sh_max + View2dUtil.V2D_SCROLLER_HANDLE_SIZE)) );
		in_min= ( (mouse <= (sh_min + View2dUtil.V2D_SCROLLER_HANDLE_SIZE)) && (mouse >= (sh_min - View2dUtil.V2D_SCROLLER_HANDLE_SIZE)) );
		in_bar= ( (mouse < (sh_max - View2dUtil.V2D_SCROLLER_HANDLE_SIZE)) && (mouse > (sh_min + View2dUtil.V2D_SCROLLER_HANDLE_SIZE)) );
		out_min= mouse < (sh_min - View2dUtil.V2D_SCROLLER_HANDLE_SIZE);
		out_max= mouse > (sh_max + View2dUtil.V2D_SCROLLER_HANDLE_SIZE);
	...
	static void scroller_activate_init(bContext C, wmOperator op, wmEvent event, short in_scroller)
	{
		...
		if (in_scroller == 'h') {
			...
			if ((v2d.keepzoom & View2dTypes.V2D_LOCKZOOM_X)!=0 && (vsm.zone == SCROLLHANDLE_MIN || vsm.zone == SCROLLHANDLE_MAX)) {
			...
			if ((v2d.keepzoom & View2dTypes.V2D_LOCKZOOM_Y)!=0 && (vsm.zone == SCROLLHANDLE_MIN || vsm.zone == SCROLLHANDLE_MAX)) {
				/* default to scroll, as handles not usable */
				vsm.zone= SCROLLHANDLE_BAR;
			}
	...
	static void scroller_activate_exit(bContext C, wmOperator op)
	{
		if (op.customdata!=null) {
			...
			vsm.v2d.scroll_ui &= ~(View2dTypes.V2D_SCROLL_H_ACTIVE|View2dTypes.V2D_SCROLL_V_ACTIVE);
	...
	static void scroller_activate_apply(bContext C, wmOperator op)
	{
		...
		switch (vsm.zone) {
			case SCROLLHANDLE_MIN:
				...
				if ((vsm.scroller == 'h') && (v2d.keepzoom & View2dTypes.V2D_LOCKZOOM_X)==0)
					v2d.cur.xmin -= temp;
				if ((vsm.scroller == 'v') && (v2d.keepzoom & View2dTypes.V2D_LOCKZOOM_Y)==0)
					v2d.cur.ymin -= temp;
				break;
			...
			default:
				...
				if ((vsm.scroller == 'h') && (v2d.keepofs & View2dTypes.V2D_LOCKOFS_X)==0) {
					v2d.cur.xmin += temp;
					v2d.cur.xmax += temp;
				}
				if ((vsm.scroller == 'v') && (v2d.keepofs & View2dTypes.V2D_LOCKOFS_Y)==0) {
					v2d.cur.ymin += temp;
					v2d.cur.ymax += temp;
				}
				break;
		...
		/* validate that view is in valid configuration after this operation */
		View2dUtil.UI_view2d_curRect_validate(v2d);
		
		/* request updates to be done... */
		Area.ED_region_tag_redraw(vsm.ar);
		View2dUtil.UI_view2d_sync(bContext.CTX_wm_screen(C), bContext.CTX_wm_area(C), v2d, View2dUtil.V2D_LOCK_COPY);
	}

	/* handle user input for scrollers - calculations of mouse-movement need to be done here, not in the apply callback! */
	public static wmOperatorType.Operator scroller_activate_modal = new wmOperatorType.Operator() {
	public int run(bContext C, wmOperator op, wmEvent event)
	{
		v2dScrollerMove vsm= (v2dScrollerMove)op.customdata;
		...
		switch (event.type) {
			...
			{
				/* calculate new delta transform, then store mouse-coordinates for next-time */
				if ((vsm.zone == SCROLLHANDLE_BAR || vsm.zone == SCROLLHANDLE_MAX)) {
			...
			case WmEventTypes.LEFTMOUSE:
				if (event.val==WmTypes.KM_RELEASE) {
					...
					if ((vsm.zone == SCROLLHANDLE_MIN_OUTSIDE || vsm.zone == SCROLLHANDLE_MAX_OUTSIDE)) {
		...
		//System.out.println("scroller_activate_modal");
	...
	public static wmOperatorType.Operator scroller_activate_invoke = new wmOperatorType.Operator() {
	public int run(bContext C, wmOperator op, wmEvent event)
	{
		ARegion ar= bContext.CTX_wm_region(C);
		View2D v2d= ar.v2d;
		...
		in_scroller= View2dUtil.UI_view2d_mouse_in_scrollers(C, v2d, event.x, event.y);
		...
		if (in_scroller!=0) {
			 ...
			 */
			if ((vsm.zone == SCROLLHANDLE_MIN || vsm.zone == SCROLLHANDLE_MAX)) {
				if ( ((vsm.scroller=='h') && (v2d.scroll & View2dTypes.V2D_SCROLL_SCALE_HORIZONTAL)==0) ||
					 ((vsm.scroller=='v') && (v2d.scroll & View2dTypes.V2D_SCROLL_SCALE_VERTICAL)==0) )
				{
			...
			if (vsm.zone == SCROLLHANDLE_BAR) {
				if ( ((vsm.scroller=='h') && (v2d.keepofs & View2dTypes.V2D_LOCKOFS_X)!=0) ||
					 ((vsm.scroller=='v') && (v2d.keepofs & View2dTypes.V2D_LOCKOFS_Y)!=0) )
				{
			...
			if ( ((vsm.scroller=='h') && (v2d.scroll & (View2dTypes.V2D_SCROLL_HORIZONTAL_HIDE|View2dTypes.V2D_SCROLL_HORIZONTAL_FULLR))!=0) ||
				 ((vsm.scroller=='v') && (v2d.scroll & (View2dTypes.V2D_SCROLL_VERTICAL_HIDE|View2dTypes.V2D_SCROLL_VERTICAL_FULLR))!=0) )
			{
				/* free customdata initialised */
				scroller_activate_exit(C, op);
			...
			if (vsm.scroller=='h')
				v2d.scroll_ui |= View2dTypes.V2D_SCROLL_H_ACTIVE;
			else
				v2d.scroll_ui |= View2dTypes.V2D_SCROLL_V_ACTIVE;
			
			/* still ok, so can add */
			WmEventSystem.WM_event_add_modal_handler(C, op);
			//System.out.println("scroller_activate_invoke");
			return WindowManagerTypes.OPERATOR_RUNNING_MODAL;
		}
		else {
	...
	public static OpFunc VIEW2D_OT_scroller_activate = new OpFunc() {
	public void run(wmOperatorType ot)
	{
		/* identifiers */
...
//	}
	...
	public static void UI_view2d_operatortypes()
	{
//		WmOperators.WM_operatortype_append(VIEW2D_OT_pan);
//		
//		WmOperators.WM_operatortype_append(VIEW2D_OT_scroll_left);
//		WmOperators.WM_operatortype_append(VIEW2D_OT_scroll_right);
//		WmOperators.WM_operatortype_append(VIEW2D_OT_scroll_up);
//		WmOperators.WM_operatortype_append(VIEW2D_OT_scroll_down);
		
//		WmOperators.WM_operatortype_append(VIEW2D_OT_zoom_in);
//		WmOperators.WM_operatortype_append(VIEW2D_OT_zoom_out);
//		
...
//	public static void UI_view2d_keymap(wmWindowManager wm)
	public static void UI_view2d_keymap(wmKeyConfig keyconf)
	{
		wmKeyMap keymap= WmKeymap.WM_keymap_find(keyconf, "View2D", 0, 0);
		
		/* pan/scroll */
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_pan", WmEventTypes.MIDDLEMOUSE, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_pan", WmEventTypes.MIDDLEMOUSE, WmTypes.KM_PRESS, WmTypes.KM_SHIFT, 0);
		
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_pan", WmEventTypes.MOUSEPAN, 0, 0, 0);
		
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_right", WmEventTypes.WHEELDOWNMOUSE, WmTypes.KM_PRESS, WmTypes.KM_CTRL, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_left", WmEventTypes.WHEELUPMOUSE, WmTypes.KM_PRESS, WmTypes.KM_CTRL, 0);
		
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_down", WmEventTypes.WHEELDOWNMOUSE, WmTypes.KM_PRESS, WmTypes.KM_SHIFT, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_up", WmEventTypes.WHEELUPMOUSE, WmTypes.KM_PRESS, WmTypes.KM_SHIFT, 0);
		
		/* zoom - single step */
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom_out", WmEventTypes.WHEELOUTMOUSE, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom_in", WmEventTypes.WHEELINMOUSE, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom_out", WmEventTypes.PADMINUS, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom_in", WmEventTypes.PADPLUSKEY, WmTypes.KM_PRESS, 0, 0);
		...
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_down", WmEventTypes.WHEELDOWNMOUSE, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_up", WmEventTypes.WHEELUPMOUSE, WmTypes.KM_PRESS, 0, 0);
			/* these may be necessary if vertical scroll is disallowed */
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_right", WmEventTypes.WHEELDOWNMOUSE, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_left", WmEventTypes.WHEELUPMOUSE, WmTypes.KM_PRESS, 0, 0);
...
//	#endif
		...
		/* zoom - drag */
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom", WmEventTypes.MIDDLEMOUSE, WmTypes.KM_PRESS, WmTypes.KM_CTRL, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom", WmEventTypes.MOUSEZOOM, 0, 0, 0);
		
		/* borderzoom - drag */
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom_border", WmEventTypes.BKEY, WmTypes.KM_PRESS, WmTypes.KM_SHIFT, 0);
		
		/* scrollers */
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroller_activate", WmEventTypes.LEFTMOUSE, WmTypes.KM_PRESS, 0, 0);

		/* Alternative keymap for buttons listview */
		keymap= WmKeymap.WM_keymap_find(keyconf, "View2D Buttons List", 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_pan", WmEventTypes.MIDDLEMOUSE, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_pan", WmEventTypes.MOUSEPAN, 0, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_down", WmEventTypes.WHEELDOWNMOUSE, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_up", WmEventTypes.WHEELUPMOUSE, WmTypes.KM_PRESS, 0, 0);
		
		RnaAccess.RNA_boolean_set(WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_down", WmEventTypes.PAGEDOWNKEY, WmTypes.KM_PRESS, 0, 0).ptr, "page", true);
		RnaAccess.RNA_boolean_set(WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroll_up", WmEventTypes.PAGEUPKEY, WmTypes.KM_PRESS, 0, 0).ptr, "page", true);
		
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom", WmEventTypes.MIDDLEMOUSE, WmTypes.KM_PRESS, WmTypes.KM_CTRL, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom", WmEventTypes.MOUSEZOOM, 0, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom_out", WmEventTypes.PADMINUS, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_zoom_in", WmEventTypes.PADPLUSKEY, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_reset", WmEventTypes.HOMEKEY, WmTypes.KM_PRESS, 0, 0);
		WmKeymap.WM_keymap_add_item(keymap, "VIEW2D_OT_scroller_activate", WmEventTypes.LEFTMOUSE, WmTypes.KM_PRESS, 0, 0);
	}
